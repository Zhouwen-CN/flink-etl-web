...
const formRules: FormRules<CreateOrUpdateTableRequestData> = {
  name: [{ required: true, trigger: "blur", message: "请输入告警名称" }],
  email: [{ required: true, trigger: "blur", message: "请输入邮箱" }],
  jobIds: [{ required: true, trigger: "blur", message: "请选择任务ID" }]
}
...
function handleUpdate(row: TableData) {
  ...
  getJobIdsDataApi(row.id).then(res => formData.value.jobIds = res.data)
}
// #endregion
...
const { dictData: jobIdSelectorData, run: getJobIdSelectorData } = useDictionary(getJobSelectorDataApi)

function handleTestSendEmail(row: TableData) {
  testSendEmailDataApi(row.id).then(() => {
    ElMessage.success("测试发送邮件成功")
  })
}
  ...
  <div class="app-container">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" @selection-change="handleSelectionChange" show-overflow-tooltip>
          ...
          <el-table-column fixed="right" label="操作" width="250" align="center">
            <template #default="scope">
              <el-button type="success" text bg size="small" @click="handleTestSendEmail(scope.row)">
                测试
              </el-button>
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="right">
        ...
        <el-form-item prop="jobId" label="任务">
          <el-select
